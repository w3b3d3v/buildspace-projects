...
```solidity
console.log("Um NFT com o ID %s foi mintada para %s", newItemId, msg.sender);
```
...
A partir de agora, toda vez que alguém mintar um NFT com essa função, será sempre o mesmo NFT - `Meu Amigo Chaves`! Nós vamos aprender nas seções daqui pra frente como mudar isso para que cada pessoa que minte um NFT obtenha um NFT aleatório e único.
...
O próximo passo é uma testnet, a qual você pode pensar como um ambiente de "testes", de "encenação". Quando nós implementarmos (deploy) em uma testnet, vamos estar habilitados a **ver nosso NFT online** e estamos um passo mais próximo de levar isso para **usuários reais**.
...
Aqui, você clica "SquareNFT" embaixo de "Collections" e boom, você vai ver os NFTs que você mintou!
