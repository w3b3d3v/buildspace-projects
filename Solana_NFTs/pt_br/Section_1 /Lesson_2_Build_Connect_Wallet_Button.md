...
Tudo bem. Já verificamos se um usuário já está conectado ao nosso aplicativo ou não. E se ele não estiver conectado? Não temos como, em nosso aplicativo, solicitar que a Phantom Wallet se conecte ao nosso aplicativo!
...
Pronto para a experiência de cadastro mais fácil da sua vida :)? Confira o código abaixo. Deixei comentários nas linhas que modifiquei.
...
Excelente! Agora você deve ter um botão de gradiente com uma aparência bem legal que diz "Conectar à carteira" renderizado em sua página.
...
```jsx
{/* Adicione a condição para mostrar isso apenas 
se não tivermos um endereço de carteira*/}
{!walletAddress && renderNotConnectedContainer()}
```
...
Lembra daquela função que configuramos, mas a qual ainda não adicionamos nenhuma lógica? É hora de adicionar a lógica de conexão à `connectWallet` :
